...
// Directory where love.js output (packaged game) will be stored by Davidobot's love.js CLI
const gameDir = path.join(__dirname, 'game_web');

app.use(express.json()); // Middleware to parse JSON bodies

// Middleware to set required HTTP headers
// These are generally good for security if SharedArrayBuffer is used by love.js
...
// Route to serve the main game page (index.html from game_web)
app.get('/', (req, res) => {
    const indexPath = path.join(gameDir, 'index.html');
    if (fs.existsSync(indexPath)) {
        res.sendFile(indexPath);
    } else {
        // Fallback to root index.html if game_web/index.html is not found (e.g. before build)
        const rootIndexPath = path.join(__dirname, 'index.html');
        if (fs.existsSync(rootIndexPath)) {
            res.sendFile(rootIndexPath);
        } else {
            res.status(404).send('Game not found. Ensure game is packaged into game_web directory or index.html exists.');
        }
    }
});
...
app.listen(port, () => {
    ...
    if (!fs.existsSync(gameDir) || !fs.existsSync(path.join(gameDir, 'index.html'))) {
        ...
        console.warn(`Make sure to run 'npm run build:lovejs' or ensure Docker build completes.`);
    }
});
